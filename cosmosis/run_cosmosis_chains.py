...
def run_chain(mode,datavec,cosmology='SLICS',sampler='multinest',
    ...
    if(cosmology=='SLICS'):
        config.set("threepoint","zMax","3.")
        config.set("threepoint","survet_area","360000.0")
        config.set("threepoint","galaxy_shapenoise","0.3654")
        config.set("threepoint","galaxy_density","30.")
        config.set("threepoint","nz_file",cosmosis_path+"inputs/nz_SLICS_euclidlike.dat")
        config.set("pipeline","values",cosmosis_path+"values.ini")
    elif(cosmology=='MR'):
        config.set("threepoint","zMax","1.1")
        config.set("threepoint","survet_area","360000.0")
        config.set("threepoint","galaxy_shapenoise","0.02")
        config.set("threepoint","galaxy_density","291.2711111")
        config.set("threepoint","nz_file",cosmosis_path+"inputs/nz_MR.dat")
    ...
    if(sampler=="multinest"):
        ...
        cosmosis_path+"outputs/results/output_"+datavec+"_"+cosmology+"_"+mode+".txt")
    ...
    config.set("threepoint_likelihood","cov_mat",cosmosis_path+"inputs/slics_cov_{}.dat".format(mode))
    config.set("threepoint_likelihood","data",cosmosis_path+"inputs/{}_{}.dat".format(datavec,mode))
    config.set("threepoint_likelihood","cov_inv",cosmosis_path+"outputs/slics_cov_inv_{}.dat".format(mode))


    if(mode=="map2"):
        config.set("threepoint","calculate_Map2","T")
        config.set("threepoint","calculate_Map3","F")
        config.set("threepoint","diag_only","F")
        config.set("threepoint_likelihood","likelihoods","Map2")
    elif(mode=="map3_diag"):
        config.set("threepoint","calculate_Map2","F")
        config.set("threepoint","calculate_Map3","T")
        config.set("threepoint","diag_only","T")
