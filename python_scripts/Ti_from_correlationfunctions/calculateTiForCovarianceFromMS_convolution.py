...
def get_one_mapmapmap_treecorr(los,
    ...
    for x,theta_ap in enumerate(theta_ap_array):
        ...
        field = ac.Map_fft_from_kappa(kappa_ms,periodic_boundary=False)
        cut_field = field[idx_cut:-idx_cut,idx_cut:-idx_cut]
        fields[x] = cut_field
    # print("Map calculation for los {} done. Resuming with T1!".format(los))
    # print("VARS: {}; {},{},{},{}".format(np.var(kappa_ms),np.var(fields[0]),np.var(fields[1]),np.var(fields[2]),np.var(fields[3])))
    ...
    for i in range(n_theta):
        ...
        np.save(fpath+"map_single_los_{}".format(los),map_single)
    ...
    for i in range(n_theta):
        for j in range(i,n_theta):
            for ii in range(n_theta):
                for jj in range(ii,n_theta):
                    savepath = fpath+"ggcorr_mapsq_{}_{}_mapsq_{}_{}_los_{}".format(theta_ap_array[i],
                                                            theta_ap_array[j],
                                                            theta_ap_array[ii],
                                                            theta_ap_array[jj],
                                                            los)
...
if(len(sys.argv)>1 and int(sys.argv[1])==1):
    ...
    calculate_multiprocessed(get_one_mapmapmap_treecorr,64,np.arange(64))
