...
#pragma omp parallel for default(shared) schedule(static)
    for (int i = 0; i < Npix2d; i++)
    {
        ...
        for (int j = 0; j < Npix2d; j++)
        {
            ...
            A13.at(i * Npix2d + j) = a13;
            A12.at(i * Npix2d + j) = a12;
            A14.at(i * Npix2d + j) = a14;
            A15.at(i * Npix2d + j) = a15;
            A16.at(i * Npix2d + j) = a16;
            A23.at(i * Npix2d + j) = a23;
            A24.at(i * Npix2d + j) = a24;
            A25.at(i * Npix2d + j) = a25;
            A26.at(i * Npix2d + j) = a26;
            A34.at(i * Npix2d + j) = a34;
            A35.at(i * Npix2d + j) = a35;
            A36.at(i * Npix2d + j) = a36;
            A45.at(i * Npix2d + j) = a45;
            A46.at(i * Npix2d + j) = a46;
            A56.at(i * Npix2d + j) = a56;
        }
    }
}
...
void totalCov(double theta1, double theta2, double theta3, double theta4, double theta5, double theta6,
              int Npix, double Apix, double sigma, const std::vector<std::complex<double>> &varthetas,
              double &t1, double &t2, double &total)
{
    int Npix2d = Npix * Npix;
    double norm = pow(Apix * sigma, 6) / Npix2d / Npix2d;
    std::vector<double> A12(Npix2d * Npix2d), A13(Npix2d * Npix2d), A14(Npix2d * Npix2d), A15(Npix2d * Npix2d), A16(Npix2d * Npix2d), A23(Npix2d * Npix2d), A24(Npix2d * Npix2d),
        A25(Npix2d * Npix2d), A26(Npix2d * Npix2d), A34(Npix2d * Npix2d), A35(Npix2d * Npix2d), A36(Npix2d * Npix2d), A45(Npix2d * Npix2d), A46(Npix2d * Npix2d), A56(Npix2d * Npix2d);
...
int main(int argc, char *argv[])
{
    if (argc != 3)
    {
        std::cerr << "Need to specify Npix and sidelength" << std::endl;
        exit(1);
    };
                ...
                for (int l = i; l < N; l++)
                {
                    ...
                    for (int m = l; m < N; m++)
                    {
                        ...
                        for (int n = m; n < N; n++)
                        {
                            ...
                            double t1, t2, total;
                            totalCov(theta1, theta2, theta3, theta4, theta5, theta6, Npix, Apix, sigma, varthetas, t1, t2, total);
