...
{
public:
    int zBins; // Number of redshift bins of n(z)
...
void * setup(cosmosis::DataBlock * options)
{
    ...
    COSMOSIS_SAFE_CALL(options->get_val(section, "zBins", config_data->zBins)); //Read number of z Bins
    COSMOSIS_SAFE_CALL(options->get_val(section, "zMax", config_data->zMax)); //Read maximal z
    read_n_of_z(fn_nz, config_data->zBins, config_data->zMax, config_data->nz); // Read in n(z) and normalize it
#if VERBOSE 
...
/**
 * @brief Execute function for cosmosis, is run for each sampling point
 * Calculates the <Map^3> for all thetas in config.thetas and stores them in the DataBlock block
 * 
 * @param block Datablock containing output from previous modules and values.ini
 * @param config Object from type data containing what was set in setup
 * @return DATABLOCK_STATUS 
 */
DATABLOCK_STATUS execute(cosmosis::DataBlock * block, void * config)
{
    ...
    {
        std::string fn_out=config_data->output_dir
                            +"/om_"+std::to_string(cosmo.om)
                            +"_sig8_"+std::to_string(cosmo.sigma8)
                            +"_h_"+std::to_string(cosmo.h)
                            +"_w_"+std::to_string(cosmo.w)+".dat"; // Outputfilename, contains current cosmology
    ...
    for (int i=0; i<N; i++)
    {
	    ...
	    std::cerr.flush();
#endif //VERBOSE
	    double Map3=apertureStatistics.MapMapMap(config_data->thetas[i]); //Do calculation
