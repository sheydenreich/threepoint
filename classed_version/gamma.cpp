...
GammaCalculator::GammaCalculator(BispectrumCalculator* Bispectrum, double prec_h, double prec_k, std::string triangle_center)
{
    ...
    z_max = Bispectrum_->get_z_max();

    if(triangle_center == "orthocenter") convert_orthocenter_to_centroid_bool = false;
    else if(triangle_center == "centroid") convert_orthocenter_to_centroid_bool = true;
    else
    {
...
std::complex<double> GammaCalculator::gamma0(double x1, double x2, double x3){
    ...
    result *= 27./8.*pow(Bispectrum_->get_om(),3)*pow(100./299792.,5); //prefactor from limber integration
    ...
    if(convert_orthocenter_to_centroid_bool) result = convert_orthocenter_to_centroid(result,x1,x2,x3,false);
...
std::complex<double> GammaCalculator::gamma1(double x1, double x2, double x3){
    ...
    if(convert_orthocenter_to_centroid_bool) result = convert_orthocenter_to_centroid(result,x1,x2,x3,true);
...
std::complex<double> GammaCalculator::integrand_gamma0_cubature(double r, double phi, double psi, double z, double x1, double x2, double x3)
{
    ...
    (integrand_gamma0_r_phi_psi_one_x(r, phi, psi, x1, x2, x3)
    +integrand_gamma0_r_phi_psi_one_x(r, phi, psi, x2, x3, x1)
    +integrand_gamma0_r_phi_psi_one_x(r, phi, psi, x3, x1, x2));
}
...
std::complex<double> GammaCalculator::integrand_r_phi_psi_gamma1(double r, double phi, double psi, double x1, double x2, double x3)
{
    ...
    double prefactor = 27./8.*pow(Bispectrum_->get_om(),3)*pow(100./299792.,5)/(3*2*pow(2*M_PI,3));
