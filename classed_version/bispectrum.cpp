...
{
  if (fabs(ell1 / ell4 - 1.) < 1e-6 && fabs(ell2 / ell5 - 1.) < 1e-6 && fabs(ell3 / ell6 - 1.) < 1e-6)
    return 1.;
  else
    return 0;
}
...
{
  if (abs(ell1 - ell2) / ell3 > 0.8 || ell3 / (ell1 + ell2) > 0.8)
    return true;
  if (abs(ell2 - ell3) / ell1 > 0.8 || ell1 / (ell2 + ell3) > 0.8)
    return true;
  if (abs(ell1 - ell3) / ell2 > 0.8 || ell2 / (ell1 + ell3) > 0.8)
    return true;
  return false;
}
...
BispectrumCalculator::BispectrumCalculator(cosmology* cosmo_arg, int n_z, double z_max_arg, bool fast_calculations_arg)
{
  n_of_z_fromFile = false;
  Pk_given = false;
  initialize(cosmo_arg, n_z, z_max_arg, fast_calculations_arg);
}

BispectrumCalculator::BispectrumCalculator(cosmology* cosmo_arg, const std::vector<double> &n_of_z, int n_z, double z_max_arg)
{
  n_of_z_fromFile = true;
  n_of_z_file = n_of_z;
  Pk_given = false;
  initialize(cosmo_arg, n_z, z_max_arg, false);
}
...
void BispectrumCalculator::initialize(cosmology* cosmo_arg, int n_z, double z_max_arg, bool fast_calculations_arg)
{
  if (initialized)
    return;
  ...
  // Allocate memory
  std::vector<double> tmp(n_redshift_bins);
  f_K_array=tmp;
  g_array=tmp;
  D1_array = tmp;
  r_sigma_array = tmp;
...
double BispectrumCalculator::bispec(double k1, double k2, double k3, double z, int idx, double didx) // non-linear BS w/o baryons [(Mpc/h)^6]
{
  ...
  if (z > 10.)
    return bispec_tree(k1, k2, k3, D1);
  ...
  logsigma8z = log10(D1 * cosmo->sigma8);
  ...
  if (alphan > 1. - (2. / 3.) * cosmo->ns)
    alphan = 1. - (2. / 3.) * cosmo->ns;
...
double BispectrumCalculator::bkappa(double ell1, double ell2, double ell3)
{
  ...
  else
  {
    ...
    double result;
    double prefactor = 27. / 8. * pow(cosmo->om, 3) * pow(H0_over_c, 5);
...
double BispectrumCalculator::P_k_nonlinear(double k, double z)
{
  ...
  a = 1.5222 + 2.8553 * n_eff + 2.3706 * nsqr + 0.9903 * n_eff * nsqr + 0.2250 * nsqr * nsqr - 0.6038 * ncur + 0.1749 * om_v * (1.0 + cosmo->w);
  a = pow(10.0, a);
  b = pow(10.0, -0.5642 + 0.5864 * n_eff + 0.5716 * nsqr - 1.5474 * ncur + 0.2279 * om_v * (1.0 + cosmo->w));
...
double BispectrumCalculator::bispectrumCovariance(double ell1, double ell2, double ell3,
                                                  ...
                                                  double survey_area)
{
  if(ell1!=ell4 || ell2!=ell5 || ell3!=ell6 || 
    delta_ell1!=delta_ell4 || delta_ell2!=delta_ell5 || delta_ell3!=delta_ell6)
    {
      std::cerr<<"Only Gaussian Bispectrum Covariance implemented, which have only a variance!"<<std::endl;
      exit(1);
    };
...
double BispectrumCalculator::integrand_bkappa(double z, ell_params p)
{
  ...
  { /* Returns bkappa_analytical if z<1, else returns 0 */
    if (z > 1)
      return 0; //fix normalization: int_0^1 f(y) dx = f(y)
    ...
    temp /= (27. / 8. * pow(cosmo->om, 3) * pow(100. / 299792., 5)); //prefactor from limber integration
...
double BispectrumCalculator::limber_integrand_power_spectrum(double ell, double z, bool nonlinear)
{
  ...
  if (nonlinear)
    ...
    return prefactor * linear_pk_at_z(ell / f_K_value, z);
}
...
double BispectrumCalculator::F2(double k1, double k2, double k3, double z, double D1, double r_sigma)
{
  ...
  logsigma8z = log10(D1 * cosmo->sigma8);
  a = 1. / (1. + z);
  omz = cosmo->om / (cosmo->om + cosmo->ow * pow(a, -3. * cosmo->w)); // Omega matter at z
...
double BispectrumCalculator::lgr_func(int j, double la, double y[2])
{
  ...
  if (j == 1)
  {
    double g, a;
    a = exp(la);
    g = -0.5 * (5. * cosmo->om + (5. - 3 * cosmo->w) * cosmo->ow * pow(a, -3. * cosmo->w)) * y[1] 
        - 1.5 * (1. - cosmo->w) * cosmo->ow * pow(a, -3. * cosmo->w) * y[0];
...
{ //assuming flat universe
  return sqrt(cosmo->om * pow(1 + z, 3) + cosmo->ow);
}
