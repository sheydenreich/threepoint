...
int main()
{
  ...
  int usteps = 15;
  int vsteps = 15;
  std::vector<double> r_array(steps);
  std::vector<double> u_array(usteps);
  std::vector<double> v_array(vsteps);
  ...
  std::vector<std::complex<double>> result_gamma0(steps * usteps * vsteps);
  std::vector<std::complex<double>> result_gamma1(steps * usteps * vsteps);
  std::vector<std::complex<double>> result_gamma2(steps * usteps * vsteps);
  std::vector<std::complex<double>> result_gamma3(steps * usteps * vsteps);
  std::vector<std::complex<double>> result_ggg(steps * usteps * vsteps);
  std::vector<std::complex<double>> result_gstargg(steps * usteps * vsteps);
  std::vector<std::complex<double>> result_ggstarg(steps * usteps * vsteps);
  std::vector<std::complex<double>> result_gggstar(steps * usteps * vsteps);
  ...
  for (int i = 0; i < steps; i++)
  {
    for (int j = 0; j < usteps; j++)
    {
      for (int k = 0; k < vsteps; k++)
      {
        ...
        double r2t = convert_angle_to_rad(r); //THIS IS THE BINNING BY JARVIS. FROM THE WEBSITE, NOT THE PAPER.
        double r3t = r2t * u;
  ...
  if (DO_SANITY_CHECKS)
  {
    ...
    double r = convert_angle_to_rad(r_array[static_cast<int>(0.7 * steps)]);
    for (int i = 0; i < 2; i++)
    {
      double r2 = (0.5 + i) * r;
  ...
  {
    {
      double r1 = convert_angle_to_rad(r_array[7]);
      double r2 = r1 * 0.8;
      double r3 = r1 * 0.6;
      ...
      std::cout << "Gamma^0(x1,x2,x3);Gamma^0(x2,x3,x1);Gamma^0(x3,x1,x2): \n"
                << real(gamma01) << "+" << imag(gamma01) << "i; " << real(gamma02) << "+" << imag(gamma02) << "i; " << real(gamma03) << "+" << imag(gamma03) << "i; \n"
                << "ggg(x1,x2,x3);ggg(x2,x3,x1);ggg(x3,x1,x2): \n"
                << real(ggg1) << "+" << imag(ggg1) << "i; " << real(ggg2) << "+" << imag(ggg2) << "i; " << real(ggg3) << "+" << imag(ggg3) << "i; \n"
                << std::endl;
      ...
      std::cout << "Gamma^1(x1,x2,x3);Gamma^2(x3,x1,x2);Gamma^3(x2,x3,x1): \n"
                << real(gamma1) << "+" << imag(gamma1) << "i; " << real(gamma2) << "+" << imag(gamma2) << "i; " << real(gamma3) << "+" << imag(gamma3) << "i; \n"
                << "gstargg(x1,x2,x3);ggstarg(x3,x1,x2);gggstar(x2,x3,x1): \n"
                << real(gstargg) << "+" << imag(gstargg) << "i; " << real(ggstarg) << "+" << imag(ggstarg) << "i; " << real(gggstar) << "+" << imag(gggstar) << "i; \n"
                << std::endl;
    }
