 ...
 */
  ...
  void initialize(uint type1, uint col1 = 8, uint nsub1 = 16, double relative_tol1 = 1e-6);
  /**
 ...
 */
  double w_double(double x, double k, uint ell_1, uint ell_2, uint i);
 ...
 */
  double A_matrix_double(uint i, uint j, double x, double k, uint ell_1, uint ell_2);
 ...
 **/
  std::vector<double> solve_LSE_single(double (*function)(double, double), double A, double B, uint col, std::vector<double> x_j, double k, uint ell);
 ...
 * power spectrum should be used. The solution to the LSE is returned as a list. 
 **/
  std::vector<double> solve_LSE_double(double (*function)(double, double), double A, double B, uint col, std::vector<double> x_j, double k, uint ell_1, uint ell_2);
 ...
 **/
  double integrate_single(double (*function)(double, double), double A, double B, uint col, double k, uint ell);
 ...
 **/
  double integrate_double(double (*function)(double, double), double A, double B, uint col, double k, uint ell_1, uint ell_2);
 ...
 **/
  double iterate_single(double (*function)(double, double), double A, double B, uint col, double k, uint ell, uint smax, bool verbose);

  /**
* Iterates over the integral by bisectiong the interval with the largest error until convergence or a maximum number of bisections, smax, is reached. 
* The final result is returned. This is for a product of two oscillatory functions.
 **/
  double iterate_double(double (*function)(double, double), double A, double B, uint col, double k, uint ell_1, uint ell_2, uint smax, bool verbose);
 ...
 */
  double levin_integrate_bessel_single(double (*function)(double, double), double k, uint ell, double a, double b);

     /**
* Returns the integral of the function \p function depending on the value for \f$ k\f$ and \f$ \ell\f$ (compare with the cases in the constructor) in the interval
* \p a and \p b for a product of oscillatory functions.
 */
  double levin_integrate_bessel_double(double (*function)(double, double), double k, uint ell_1, uint ell_2, double a, double b);
};
