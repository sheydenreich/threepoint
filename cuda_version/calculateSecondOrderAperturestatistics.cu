...
Argument 5: Shapenoise: 0 off, 1 on
...
./calculateApertureStatistics.x ../necessary_files/MR_cosmo.dat ../necessary_files/HOWLS_covariance.dat ../necessary_files/HOWLS_thetas.dat ../../results_MR/MapMapMap_bispec_gpu_nz.dat 1 1 ../necessary_files/nz_MR.dat
  ...
  std::cerr << covPar;
  if(shapenoise)
    std::cerr << "Calculating Power Spectrum WITH shapenoise" << std::endl;
  else
  ...
  // Calculation only for theta1<=theta2<=theta3
    for (int i = 0; i < N; i++) 
    {
        double theta = thetas.at(i) * 3.1416 / 180. / 60; // Conversion to rad

        std::cout << step << "/" << N << ": Theta:" << thetas.at(i) << " \r";
        std::cout.flush();

        double Map2_here =
            Map2(theta,covPar,shapenoise); // Do calculation
