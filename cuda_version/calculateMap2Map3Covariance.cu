...
Argument 12: Survey geometry, either circle, square, infinite, or rectangular
  ...
  bool calculate_T2 = std::stoi(argv[6]);
  bool calculate_T3 = std::stoi(argv[7]);
  ...
  std::vector<double> Cov_term2s, Cov_term3s, Cov_term4s;
  ...
  int N_ind = theta_combis.size(); // Number of independent theta-combinations for Map3
  int N_total = N_ind * N;         // Number of elements in covariance
  ...
  // std::vector<double> thetas_c={convert_angle_to_rad(2), convert_angle_to_rad(2), convert_angle_to_rad(2)};
  // double theta4=convert_angle_to_rad(8);
  // double term=Map2Map3_T2_total(thetas_c, theta4);
  // std::cerr<<term<<std::endl;

  for (int i = 0; i < N_ind; i++) // Go over Map3 thetas
  {
    for (int j = 0; j < N; j++) // Go over Map2 thetas
    {
      double theta4 = convert_angle_to_rad(thetas.at(j));
      try
      {
        if (calculate_T2)
        {
          double term2 = Map2Map3_T2_total(theta_combis.at(i), theta4);
          Cov_term2s.push_back(term2);
        };
        if (calculate_T3)
        {
          double term3 = Map2Map3_T3_total(theta_combis.at(i), theta4);
          Cov_term3s.push_back(term3);
        };
        if (calculate_T4)
        {
          double term4 = Map2Map3_T4_total(theta_combis.at(i), theta4);
      ...
      fprintf(stderr, "\r [%3d%%] in %.2f h. Est. remaining: %.2f h. Average: %.2f s per step. Last thetas: (%.2f, %.2f, %.2f, %.2f) [%s]",
              ...
              convert_rad_to_angle(theta4), "arcmin");
    }
  }
  ...
  if (calculate_T2)
  {
    sprintf(filename, "covMap2Map3_%s_term2Numerical_sigma_%.2f_n_%.2f_thetaMax_%.2f_gpu.dat",
            type_str.c_str(), sigma, n_deg, thetaMax_deg);
    std::cerr << "Writing Term2 to " << out_folder + filename << std::endl;

    try
    {
      writeCrossCov(Cov_term2s, N, N_ind, out_folder + filename);
    }
    catch (const std::exception &e)
    {
      std::cerr << e.what() << '\n';
      std::cerr << "Writing instead to current directory!" << std::endl;
      writeCrossCov(Cov_term2s, N, N_ind, filename);
    }
  };
    ...
    if (calculate_T3)
  {
    sprintf(filename, "covMap2Map3_%s_term3Numerical_sigma_%.2f_n_%.2f_thetaMax_%.2f_gpu.dat",
            type_str.c_str(), sigma, n_deg, thetaMax_deg);
    std::cerr << "Writing Term3 to " << out_folder + filename << std::endl;

    try
    {
      writeCrossCov(Cov_term3s, N, N_ind, out_folder + filename);
    }
    catch (const std::exception &e)
    {
      std::cerr << e.what() << '\n';
      std::cerr << "Writing instead to current directory!" << std::endl;
      writeCrossCov(Cov_term3s, N, N_ind, filename);
    }
  };
  ...
  if (calculate_T4)
  {
    sprintf(filename, "covMap2Map3_%s_term4Numerical_sigma_%.2f_n_%.2f_thetaMax_%.2f_gpu.dat",
            type_str.c_str(), sigma, n_deg, thetaMax_deg);
    std::cerr << "Writing Term4 to " << out_folder + filename << std::endl;

    try
    {
      writeCrossCov(Cov_term4s, N, N_ind, out_folder + filename);
    }
    catch (const std::exception &e)
    {
      std::cerr << e.what() << '\n';
      std::cerr << "Writing instead to current directory!" << std::endl;
      writeCrossCov(Cov_term4s, N, N_ind, filename);
    }
  };
  return 0;
}
